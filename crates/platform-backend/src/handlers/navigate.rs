...
pub async fn get_home_overview(
    ...
    let mut events = ctx
        .event_emitter
        .emit_signals(&[signals::ACTIVE_PAGE_OVERVIEW, &i18n_signals]);
...
pub async fn get_home_movies(
    ...
    let mut events = ctx
        .event_emitter
        .emit_signals(&[signals::ACTIVE_PAGE_MOVIES, &i18n_signals]);
    events.extend(ctx.event_emitter.emit_elements(&patches));
    sse_response(&events)
}
...
pub async fn get_home_series(
    ...
    let mut events = ctx
        .event_emitter
        .emit_signals(&[signals::ACTIVE_PAGE_SERIES, &i18n_signals]);
    events.extend(ctx.event_emitter.emit_elements(&patches));
    sse_response(&events)
}
